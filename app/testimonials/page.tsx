...
function Stars({ rating, size = 'sm' }: { rating: number; size?: 'sm' | 'lg' }) {
  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star key={i} className={`${cls} ${i <= rating ? 'fill-yellow-400 text-yellow-400' : 'text-[var(--color-neutral-300)]'}`} />
      ))}
    </div>
  );
}
    ...
    <>
      <section className="bg-[var(--color-primary-900)] text-white py-20">
        <div className="container mx-auto px-6 text-center">
          ...
          <h1 className="text-5xl lg:text-6xl font-bold mb-4" style={{ fontFamily: 'var(--font-playfair), serif' }}>
            What Our Clients Say
          </h1>
          <p className="text-xl text-white/70 max-w-2xl mx-auto mb-8">
            Thousands of satisfied clients across DC, Maryland & Virginia have trusted Federal Title with their most important financial transaction.
          </p>
          ...
          {place ? (
            <div className="inline-flex items-center gap-4 bg-white/10 rounded-2xl px-8 py-4">
              <div className="text-center">
                ...
                <p className="text-white/60 text-sm mt-1">{place.userRatingCount.toLocaleString()} Google reviews</p>
              </div>
            </div>
          ) : (
          ...
          {place?.reviews?.length ? (
            <>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                {place.reviews.map((review, i) => (
                  <a
                    ...
                    className="bg-white rounded-xl border border-[var(--color-neutral-200)] p-7 flex flex-col hover:border-[var(--color-accent-300)] hover:shadow-md transition-all"
                  >
              ...
              <div className="text-center">
                ...
                >
                  See all {place.userRatingCount.toLocaleString()} reviews on Google →
                </a>
              </div>
            </>
          ) : (
            <div className="text-center py-16 text-[var(--color-neutral-500)]">
              Reviews unavailable. Please check back shortly.
            </div>
          )}
        </div>
      </section>
